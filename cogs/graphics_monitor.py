...
class DateParseResult(NamedTuple):
    ...
    in_effect_datetime: Optional[
        datetime.datetime
    ]  # When the graphic goes "in effect" (start time)
...
class DateParser:
    ...
    DATE_RANGE_PATTERN_WITH_YEAR_FULL = r"(\d{1,2})\.(\d{1,2})\.(\d{4})\s*-\s*(\d{1,2})\.(\d{1,2})\.(\d{4})"  # DD.MM.YYYY-DD.MM.YYYY (spaces around dash optional)
    MONTHS = {
        # English month names
        "january": 1,
        "february": 2,
    ...
    def parse_date(cls, content: str) -> DateParseResult:
        ...
        if match:
            day1, month1, year1_short, day2, month2, year2_short = map(
                int, match.groups()
            )
            ...
            try:
                ...
                start_date = datetime.datetime(
                    year1, month1, day1, tzinfo=datetime.timezone.utc
                )
                end_date = datetime.datetime(
                    year2, month2, day2, tzinfo=datetime.timezone.utc
                )

                # Add 1 day grace period and set to end of day
                expiry = end_date.replace(
        ...
        match = re.search(cls.DATE_RANGE_PATTERN_WITH_YEAR_FULL, content)
        if match:
            day1, month1, year1_full, day2, month2, year2_full = map(
                int, match.groups()
            )
            try:
                # Create start and end dates
                start_date = datetime.datetime(
                    year1_full, month1, day1, tzinfo=datetime.timezone.utc
                )
                end_date = datetime.datetime(
                    year2_full, month2, day2, tzinfo=datetime.timezone.utc
                )
        ...
        if match:
            ...
            try:
                # Create start and end dates
                start_date = datetime.datetime(
                    start_year_to_use, month1, day1, tzinfo=datetime.timezone.utc
                )
                end_date = datetime.datetime(
                    end_year_to_use, month2, day2, tzinfo=datetime.timezone.utc
                )
        ...
        if match:
            ...
            start_date = datetime.datetime(
                year_to_use, month_num, 1, tzinfo=datetime.timezone.utc
            )
            ...
            if month_num == 12:
                ...
                last_day = 31
                end_date = datetime.datetime(
                    year_to_use, 12, 31, 23, 59, 59, tzinfo=datetime.timezone.utc
                )
            else:
                # Get first day of next month, then subtract 1 day
                next_month = datetime.datetime(
                    year_to_use, month_num + 1, 1, tzinfo=datetime.timezone.utc
                )
            ...
            display_name = cls.MONTH_DISPLAY_NAMES.get(
...
class GraphicsMonitorCog(commands.GroupCog, group_name="graphics"):
    ...
    async def check_expired_graphics(self):
        ...
        try:
            ...
            expired_graphics = (
                self.session.query(MonitoredGraphic)
                .filter(
    ...
    def _calculate_reminder_time(
        self,
        in_effect_datetime: datetime.datetime,
    ...
    async def _send_reminder(self, graphic: MonitoredGraphic):
        ...
        try:
            ...
            if not channel:
                logger.warning(
                    f"Channel {graphic.channel_id} not found for graphic {graphic.message_id}"
                )
            ...
            try:
                ...
                logger.info(
                    f"Original message {graphic.message_id} deleted, removing from monitoring"
                )
            ...
            try:
                ...
                logger.info(
                    f"Sent reminder for message {graphic.message_id} (reminder message: {reminder_message.id})"
                )

            except discord.Forbidden:
                logger.error(
                    f"Cannot send message or add reaction in channel {graphic.channel_id}"
                )
            ...
            logger.error(
                f"Error sending reminder for graphic {graphic.message_id}: {e}",
                exc_info=True,
            )
    ...
    async def _request_deletion_approval(self, graphic: MonitoredGraphic):
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"Message {graphic.message_id} already deleted, removing from monitoring"
                )
                self.session.delete(graphic)
                self.session.commit()
                return
            ...
            embed.add_field(
                name="Date Range", value=graphic.date_format or "N/A", inline=True
            )
            embed.add_field(
                name="Expired",
                value=graphic.expiry_date.strftime("%Y-%m-%d %H:%M UTC")
            ...
            try:
                ...
                logger.info(
                    f"Sent deletion approval request to moderator for message {graphic.message_id}"
                )
    ...
    async def _mark_no_response(
        self, graphic: MonitoredGraphic, message: Message = None
    ):
        ...
        try:
            ...
            logger.error(
                f"Error adding reaction to message {graphic.message_id}: {e}",
                exc_info=True,
            )

    async def handle_deletion_approval(
        self, graphic: MonitoredGraphic, approved: bool, interaction: Interaction
    ):
        ...
        try:
            ...
            try:
                ...
                await interaction.response.send_message(
                    "Message has already been deleted.", ephemeral=True
                )
                self.session.delete(graphic)
                self.session.commit()
            ...
            if graphic.reminder_message_id:
                try:
                    reminder_message = await channel.fetch_message(
                        graphic.reminder_message_id
                    )
                    ...
                    logger.info(
                        f"Reminder message {graphic.reminder_message_id} already deleted"
                    )
                except Exception as e:
                    logger.error(f"Error deleting reminder message: {e}", exc_info=True)
            ...
            if approved:
                ...
                await message.delete()
                await interaction.response.send_message(
                    f"✅ Message deleted from {channel.mention}.", ephemeral=True
                )
                logger.info(
                    f"Deleted message {graphic.message_id} after moderator approval"
                )
            else:
                # Keep the message, remove from monitoring
                await interaction.response.send_message(
                    f"✅ Message kept in {channel.mention} and removed from monitoring.",
                    ephemeral=True,
                )
            ...
            await interaction.response.send_message(
                f"Error processing request: {e}", ephemeral=True
            )

    async def _get_date_from_message_or_reference(
        self, message: Message
    ) -> DateParseResult:
        """
        Try to parse a date from the message's own content first.
        If that fails and the message is a reply/quote, fall back to the
        referenced message's content.
        """
        # 1. Try the message's own content
        parse_result = DateParser.parse_date(message.content)
        if parse_result.original_date_string and parse_result.expiry_datetime:
            return parse_result

        # 2. Fall back to the referenced (replied-to / quoted) message
        if message.reference and message.reference.message_id:
            try:
                ref_msg = message.reference.resolved
                if not ref_msg:
                    ref_channel_id = message.reference.channel_id or message.channel.id
                    ref_channel = self.bot.get_channel(ref_channel_id)
                    if ref_channel:
                        ref_msg = await ref_channel.fetch_message(
                            message.reference.message_id
                        )

                if ref_msg and ref_msg.content:
                    ref_parse_result = DateParser.parse_date(ref_msg.content)
                    if (
                        ref_parse_result.original_date_string
                        and ref_parse_result.expiry_datetime
                    ):
                        logger.info(
                            f"Date found in referenced message {ref_msg.id} "
                            f"(reply: {message.id})"
                        )
                        return ref_parse_result
            except discord.NotFound:
                logger.warning(
                    f"Referenced message {message.reference.message_id} not found "
                    f"while processing reply {message.id}"
                )
            except discord.Forbidden:
                logger.warning(
                    f"No permission to fetch referenced message "
                    f"{message.reference.message_id}"
                )
            except Exception as e:
                logger.error(
                    f"Error fetching referenced message for {message.id}: {e}",
                    exc_info=True,
                )
    ...
    async def on_message(self, message: Message):
        ...
        monitored_channel = (
            self.session.query(MonitoredGraphicsChannel)
        ...
        # Try to parse date from message content (falls back to referenced message if needed)
        parse_result = await self._get_date_from_message_or_reference(message)
        ...
        if parse_result.original_date_string and parse_result.expiry_datetime:
            ...
            if parse_result.in_effect_datetime:
                reminder_time = self._calculate_reminder_time(
                    parse_result.in_effect_datetime, message.created_at
                )
            ...
            reminder_info = (
                f", reminder at: {reminder_time}" if reminder_time else ", no reminder"
            )
            logger.info(
                f"Automatically added graphic {message.id} to monitoring (expires: {parse_result.expiry_datetime}{reminder_info})"
            )
    ...
    async def on_message_edit(self, before: Message, after: Message):
        ...
        graphic = (
            self.session.query(MonitoredGraphic).filter_by(message_id=after.id).first()
        )
        if not graphic:
            return
        ...
        if graphic.reminder_sent:
            logger.info(
                f"Skipping reminder update for {after.id} - reminder already sent"
            )
            return

        # Re-parse the date from updated content (falls back to referenced message if needed)
        parse_result = await self._get_date_from_message_or_reference(after)
        ...
        if parse_result.original_date_string and parse_result.expiry_datetime:
            ...
            if parse_result.in_effect_datetime:
                reminder_time = self._calculate_reminder_time(
                    parse_result.in_effect_datetime, after.created_at
                )
            ...
            logger.info(
                f"Updated graphic {after.id} after edit (new expiry: {parse_result.expiry_datetime}, reminder: {reminder_time})"
            )
        else:
            # No valid date format in edited message - remove from monitoring and delete reminder if exists
            logger.info(
                f"No valid date in edited message {after.id}, removing from monitoring"
            )
            ...
            if graphic.reminder_message_id:
                try:
                    ...
                    if channel:
                        ...
                        await reminder_message.delete()
                        logger.info(
                            f"Deleted reminder message {graphic.reminder_message_id} after edit"
                        )
                except discord.NotFound:
                    pass
                except Exception as e:
    ...
    async def _request_date_format(self, message: Message):
        """
        Ask for a date format for a message that could not be parsed automatically.
        Who gets asked depends on the notify_author_on_date_missing feature flag:
        - False (default): DM the configured moderator.
        - True: DM the message author; fall back to the moderator if the author
                has DMs disabled.
        """
        try:
            ...
            if message.id in self.pending_date_requests:
                logger.info(
                    f"Date format request already pending for message {message.id}"
                )
                return

            if self.notify_author_on_date_missing:
                await self._send_date_format_request_to_author(message)
            else:
                await self._send_date_format_request_to_moderator(message)

        except Exception as e:
            logger.error(f"Error requesting date format: {e}", exc_info=True)

    def _build_date_format_embed(
        self, message: Message, *, for_author: bool = False
    ) -> discord.Embed:
        """Build the embed used in date-format request DMs."""
        if for_author:
            embed = discord.Embed(
                title="Date Format Needed for Your Post",
                description=(
                    f"Your post in {message.channel.mention} does not contain a "
                    f"recognized date format, so it cannot be tracked automatically. "
                    f"Please edit your message to include a date range, or use the "
                    f"buttons below to add one manually."
                ),
            ...
            embed = discord.Embed(
                ...
                description=(
                    f"A graphic was posted in {message.channel.mention} without a "
                    f"recognized date format."
                ),
                color=discord.Color.blue(),
            )
    ...
    async def _send_date_format_request_to_moderator(self, message: Message):
        """DM the moderator asking for a date format (original behaviour)."""
        try:
            moderator = await self.bot.fetch_user(self.moderator_id)
            embed = self._build_date_format_embed(message, for_author=False)
            view = DateRequestView(self, message)
            await moderator.send(embed=embed, view=view)
            self.pending_date_requests[message.id] = message
            logger.info(
                f"Sent date format request to moderator for message {message.id}"
            )
        except discord.Forbidden:
            logger.error(f"Cannot send DM to moderator {self.moderator_id}")

    async def _send_date_format_request_to_author(self, message: Message):
        """DM the message author asking for a date format.
        Falls back to the moderator if the author has DMs disabled."""
        try:
            embed = self._build_date_format_embed(message, for_author=True)
            view = DateRequestView(self, message)
            await message.author.send(embed=embed, view=view)
            self.pending_date_requests[message.id] = message
            logger.info(
                f"Sent date format request to author {message.author.id} "
                f"for message {message.id}"
            )
        except discord.Forbidden:
            logger.warning(
                f"Cannot send DM to author {message.author.id} "
                f"(DMs disabled) — falling back to moderator for message {message.id}"
            )
            await self._send_date_format_request_to_moderator(message)

    @app_commands.command(
        name="enable-channel", description="Enable graphics monitoring for a channel"
    )
    @app_commands.describe(
        channel="The channel to monitor (defaults to current channel)"
    )
    async def enable_channel(
        ...
        monitored_channel = MonitoredGraphicsChannel(
            channel_id=channel.id, guild_id=interaction.guild_id
        )
        self.session.add(monitored_channel)
        self.session.commit()

        await interaction.response.send_message(
            f"✅ Graphics monitoring enabled for {channel.mention}.", ephemeral=True
        )
        logger.info(f"Enabled graphics monitoring for channel {channel.id}")

    @app_commands.command(
        name="disable-channel", description="Disable graphics monitoring for a channel"
    )
    @app_commands.describe(
        channel="The channel to stop monitoring (defaults to current channel)"
    )
    async def disable_channel(
        ...
        graphics = (
            self.session.query(MonitoredGraphic).filter_by(channel_id=channel.id).all()
        )
    ...
    @app_commands.command(
        name="list-monitored-graphics", description="List all graphics being monitored"
    )
    async def list_monitored_graphics(self, interaction: Interaction):
        """List all monitored graphics and their expiry times"""
        graphics = (
            self.session.query(MonitoredGraphic)
            .order_by(MonitoredGraphic.expiry_date)
            .all()
        )

        if not graphics:
            await interaction.response.send_message(
                "No graphics are currently being monitored.", ephemeral=True
            )
            return
    ...
    @app_commands.command(
        name="add-graphics-monitor", description="Add a message to graphics monitoring"
    )
    @app_commands.describe(
    ...
    async def add_graphics_monitor(
        self,
        ...
        link_match = re.match(
            r"https://(?:discord\.com|ptb\.discord\.com|canary\.discord\.com)/channels/\d+/(\d+)/(\d+)",
            message_reference,
        )
        if link_match:
            ...
            if not target_channel:
                await interaction.response.send_message(
                    f"Could not find channel with ID {channel_id}.", ephemeral=True
                )
                return
        else:
        ...
        existing = (
            self.session.query(MonitoredGraphic).filter_by(message_id=msg_id).first()
        )
        if existing:
            await interaction.response.send_message(
                f"Message {msg_id} is already being monitored.", ephemeral=True
            )
            return
        ...
        if parse_result.in_effect_datetime:
            ...
            reminder_time = self._calculate_reminder_time(
                parse_result.in_effect_datetime, command_time
            )
        ...
        reminder_text = (
            f"\n**Reminder:** {reminder_time.strftime('%Y-%m-%d %H:%M UTC')}"
            if reminder_time
        ...
        logger.info(
            f"Manually added graphic {msg_id} to monitoring (expires: {parse_result.expiry_datetime}, reminder: {reminder_time})"
        )

    @app_commands.command(
        name="remove-graphics-monitor",
        description="Remove a message from graphics monitoring",
    )
    ...
    async def remove_graphics_monitor(self, interaction: Interaction, message_id: str):
        ...
        if not graphic:
            await interaction.response.send_message(
                f"Message {msg_id} is not being monitored.", ephemeral=True
            )
            return
        ...
        await interaction.response.send_message(
            f"✅ Removed message {msg_id} from monitoring.", ephemeral=True
        )
...
class DateInputModal(discord.ui.Modal, title="Add Date Format"):
    ...
    async def on_submit(self, interaction: Interaction):
        ...
        try:
            ...
            await interaction.response.send_message(
                "❌ Message has been deleted and cannot be monitored.", ephemeral=True
            )
            # Clean up tracking
            ...
            await interaction.response.send_message(
                "❌ I no longer have access to that channel.", ephemeral=True
            )
            return

        # Check if already being monitored
        existing = (
            self.cog.session.query(MonitoredGraphic)
            .filter_by(message_id=self.message.id)
            .first()
        )
        ...
        if not parse_result.original_date_string or not parse_result.expiry_datetime:
            ...
            error_embed.add_field(
                name="Supported Formats", value=SUPPORTED_DATE_FORMATS, inline=False
            )
            error_embed.add_field(
                name="Message Link",
            ...
            logger.info(
                f"Invalid date format provided for message {self.message.id}: {date_string}"
            )
            return
        ...
        success_embed.add_field(
            name="Channel", value=self.message.channel.mention, inline=True
        )
        success_embed.add_field(
            name="Date Range", value=parse_result.original_date_string, inline=True
        )
        success_embed.add_field(
            name="Expires",
        ...
        logger.info(
            f"Added graphic {self.message.id} to monitoring via modal (expires: {parse_result.expiry_datetime})"
        )
...
class DateRequestView(discord.ui.View):
    ...
    @discord.ui.button(
        label="Skip Monitoring",
        style=discord.ButtonStyle.secondary,
        emoji="⏭️",
        custom_id="skip_monitoring",
    )
    ...
    async def view_button(self, interaction: Interaction, button: discord.ui.Button):
        ...
        view_embed.add_field(
            name="Author", value=self.message.author.mention, inline=True
        )

        if self.message.content:
            preview = (
                self.message.content[:500] + "..."
                if len(self.message.content) > 500
...
class ApprovalView(discord.ui.View):
    ...
    @discord.ui.button(
        label="Delete Message", style=discord.ButtonStyle.danger, custom_id="delete_yes"
    )
    ...
    @discord.ui.button(
        label="Keep Message", style=discord.ButtonStyle.success, custom_id="delete_no"
    )
